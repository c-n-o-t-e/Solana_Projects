...
import { TokenLotteryContract } from "../target/types/token_lottery_contract";
...
describe("token-lottery", () => {
  ...
  const program = anchor.workspace.TokenLotteryContract as Program<TokenLotteryContract>;
  ...
  async function buyTicket() {
    ...
    const balance = await connection.getBalance(token_lottery);
    ...
    const balance1 = await connection.getBalance(token_lottery);
    console.log("buy ticket ", sig, balance1);
  }
  ...
  it("Is initialized!", async () => {
    ...
    const blockhashContext1 = await connection.getLatestBlockhash();
    const tx1 = new anchor.web3.Transaction({
      blockhash: blockhashContext1.blockhash,
      lastValidBlockHeight: blockhashContext1.lastValidBlockHeight,
      feePayer: wallet.payer.publicKey,
    }).add(initLotteryIx)

    const sig1 = await anchor.web3.sendAndConfirmTransaction(connection, tx1, [wallet.payer], {skipPreflight:true});
    console.log(sig1);
  });
